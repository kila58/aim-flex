...
namespace enetpp {
	...
	public:
		...
		// :::tfffeqw:::aSD;f::::::::::111111112eee
		client_connect_params autism3;
		static void autism1(client* autism2)
		{
			const void * address = static_cast<const void*>(autism2);
			std::stringstream ss;
			ss << address;
			std::string name = ss.str();

			autism2->trace("autism2: " + name);
			autism2->trace(autism2->autism3._server_host_name);

			autism2->run_in_thread(autism2->autism3);
		}
		...
		void connect(const client_connect_params& params) {
			...
			//CreateThread(NULL, NULL, this->run_in_thread, NULL, NULL, NULL);

			//CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)test, NULLULL, NULL));
		...
		void disconnect() {
			if (_thread != nullptr) {
				...
				// todo: implement a WaitForObjects way to do this
				// wait 100ms for main thread to receive disconnect command
				// then this thread will clean up itself
				//Sleep(100);
