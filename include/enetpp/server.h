...
namespace enetpp {
	...
	public:
		...
		void disconnect_client_now(ClientT* client)
		{
			for (auto it = _thread_peer_map.begin(); it != _thread_peer_map.end(); )
			{
				if (it->first == client->get_id())
				{
					enet_peer_disconnect_now(it->second, 0);
					it->second->data = nullptr;

					it = _thread_peer_map.erase(it);
				}
				else
					++it;
			}
